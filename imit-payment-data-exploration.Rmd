...
Get the raw data from the consolidated-revenue-fund-detailed-schedules-of-payments-other-supplier-payments record:
...
payments_raw <- payments_resources_df  %>% 
  filter(location != "bcgeographicwarehouse") %>% 
  # slice(1:2) %>% 
  select(id) %>% 
  pull() %>% 
map_df(~ bcdc_get_data(record = "f6bad1d5-d1dc-4fdd-a076-650f55cc3fbc",
                        resource = .))
